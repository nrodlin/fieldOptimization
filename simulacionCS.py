...
for s in range(0, nSamples, image_sampling):
    ...
    for dir in range(nDirs):
        ...
        #                                  np.mean(sun_image_aberrated_perdir_OL[dir, index, tt_margin:-tt_margin, tt_margin:-tt_margin])#laplacian.var()
    index += 1
...
# Compute CL
index = 0
uprev = np.zeros((delay + 1, rmx_glao.shape[0]))
...
for s in range(nSamples):
    if s < delay:  # before the delay sample, the system does not respond to any variation
        ...
        # Compute error
        error = modes_dir[:, s-delay, :] - uprev[-1 * (delay + 1)]
        # Compute common error term
        sgn_modes = np.sign(error)
        Krn = np.all(error > 0, axis=0) | np.all(error <= 0, axis=0)  # logic product

        error_common = np.min(error, axis=0) * sgn_modes[0,:] * Krn
        # Compute residual weights
        Cbest = np.max(contrast_CL_perdir[:, index])
        Cworst = np.min(contrast_CL_perdir[:, index])

        if (Cbest - Cworst == 0):
            weights = np.ones_like(error[:, 0])
        else:
            weights = np.ones_like(error[:, 0]) - ((np.squeeze(contrast_CL_perdir[:, index].T) - Cworst) / (Cbest - Cworst))

        # weights /= np.sum(weights)

        error_res = np.average(error - error_common, axis=0, weights=weights)
        # Compute control action
        u = uprev[-1, :] * decay + gain * (error_common + error_res)  # modal correction, negative feedback is explicitly included
...
# fits.writeto('cs_results_sun.fits', sun_image_aberrated_perdir_CL, overwrite=True)
