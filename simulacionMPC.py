...
# MPC variables
nModes = rmx_glao.shape[0]
NmpcHist = 6
Mhorizon = 3

modes_hist_mpc = np.zeros((nDirs, NmpcHist, rmx_glao.shape[0])) # 0 is the oldest element, Nhist the newest
u_hist_mpc = np.zeros((NmpcHist+1, rmx_glao.shape[0]))
x_hist = np.array([i for i in range(NmpcHist)])

modes_pred_mpc= np.zeros((nDirs, Mhorizon, rmx_glao.shape[0]))
x_pred = np.array([NmpcHist + i for i in range(NmpcHist)])

uopt = np.squeeze(u_hist_mpc[0, :])
A_coeff = np.zeros((nDirs, nModes))
B_coeff = np.zeros((nDirs, nModes))

for s in range(nSamples):
    if s < delay:
        uopt = u_hist_mpc[-1, :]
    else:
        # Get current modal measuremente -- This will be RMX * slopes for the bench
        phi_turbulence = modes_dir[:, s - delay, :]
        # Buffer update
        # Roll stack
        modes_hist_mpc = np.roll(modes_hist_mpc, -1, axis=1)
        u_hist_mpc = np.roll(u_hist_mpc, -1, axis=0)
        # Substitute oldest element
        modes_hist_mpc[:, -1, :] = phi_turbulence
        u_hist_mpc[-1, :] = uopt

        # Compute regression and optimal control sequence -- Only if the buffer is full
        if s > (NmpcHist + delay):
            for dir in range(nDirs):
                for mode in range(nModes):
                    num = (NmpcHist*np.sum(x_hist*modes_hist_mpc[dir, :, mode]) - np.sum(x_hist)*np.sum(modes_hist_mpc[dir, :, mode]))
                    den = (NmpcHist * np.sum(x_hist**2) - np.sum(x_hist)**2)

                    A_coeff[dir, mode] = num / den

                    num = np.sum(modes_hist_mpc[dir, :, mode])*np.sum(x_hist**2) - np.sum(x_hist) * np.sum(x_hist*modes_hist_mpc[dir, :, mode])
                    B_coeff[dir, mode] = num / den
            # Predict next error
            for step in range(Mhorizon):
                for dir in range(nDirs):
                    for mode in range(nModes):
                        modes_pred_mpc[dir, step, mode] = A_coeff[dir, mode] * x_pred[step] + B_coeff[dir, mode]
            # Compute optical command
            uopt = opt.minimize(lambda uopt: np.sum((1/2) * (modes_pred_mpc[:, delay-1, :] - uopt)**2), u_hist_mpc[-1, :]).x
    # Compute residual per dir
    for dir in range(nDirs):
        modes_res_CL_perdir_glao[dir, s, :] = modes_dir[dir, s, :] - uopt

print('Time MPC: ' + str(time.time() - t) + '[s]')
t = time.time()

for s in range(nSamples):
    if (s % image_sampling) == 0:
        for dir in range(nDirs):
            index = s // image_sampling
            ...
            sun_image_aberrated_t = np.fft.fftshift(
                np.real(np.fft.ifft2(np.fft.fft2(sun_image) * np.fft.fft2(psf))))

            sun_image_aberrated_perdir_CL[dir, index, :, :] = sun_image_aberrated_t[rini:rend, cini:cend]

            # laplacian = cv2.Laplacian(
            #     sun_image_aberrated_perdir_CL[dir, index, tt_margin:-tt_margin, tt_margin:-tt_margin].astype(float),
            #     cv2.CV_64F)

            contrast_CL_perdir[dir, index] = np.std(
                sun_image_aberrated_perdir_CL[dir, index, tt_margin:-tt_margin, tt_margin:-tt_margin]) / \
                                             np.mean(sun_image_aberrated_perdir_CL[dir, index, tt_margin:-tt_margin,
                                                     tt_margin:-tt_margin])

            # contrast_CL_perdir[dir, index] = laplacian.var()

fits.writeto('mpc_results_scao_sun.fits', sun_image_aberrated_perdir_CL, overwrite=True)
# fits.writeto('mpc_results_sun.fits', sun_image_aberrated_perdir_CL, overwrite=True)
...
print(np.mean(contrast_CL_perdir[:, 3:], axis=1))
print('End')